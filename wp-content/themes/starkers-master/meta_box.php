...
class custom_add_meta_box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			...
			if ( in_array( true, array(
				...
				wp_register_script( 'chosen', get_template_directory_uri(). '/js/chosen.js', array( 'jquery' ) );
				$deps[] = 'chosen';
			...
			if ( in_array( true, array( 
				...
				wp_enqueue_script( 'meta_box', get_template_directory_uri(). '/js/scripts.js', $deps );
			
			// css
			$deps = array();
			wp_register_style( 'jqueryui', get_stylesheet_directory_uri() . '/css/jqueryui.css' );
			if ( meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jqueryui';
			if ( meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			wp_enqueue_style( 'meta_box', get_stylesheet_directory_uri(). '/css/meta_box.css', $deps );
		}
	}
	 ...
	 *create data base tables for movies
	 */
...
$fields = array(
	array( // Select box
		'label'	=> 'Select Cinema Room', // <label>
		'desc'	=> '', // description
		'id'	=> $prefix.'select', // field id and name
		'type'	=> 'select', // type of field
		'options' => array ( // array of options
			'cineA' => array ( // array key needs to be the same as the option value
				'label' => 'cineA', // text displayed as the option
				'value'	=> 'cineA' // value stored for the option
			),
			'cineB' => array (
				'label' => 'cineB',
				'value'	=> 'cineB'
...
$sample_box = new custom_add_meta_box( 'sample_box', 'Movie Details Part B', $fields, 'movie', true );
...
array( // Image ID field
		...
		'desc'	=> '', // description
		'id'	=> $my.'image', // field id and name
		'type'	=> 'image' // type of field
	)
);
$home_box=new custom_add_meta_box( 'home_box', 'Home Dynamic Content', $homefields, 'page', true );
