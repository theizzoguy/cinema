...
/***************************ajax play trailer***********************/
	...
	$('.bxslider').on('click','.synopsis', function(e){
		var $movie_a=$(this).children('a');
		var  movie =$movie_a.attr('class');
		//splt movie name and add +
	...
	$('.bxslider').on('click','.synopsis', function(){
		 $trailer=$(this).children('a');
		 $name=$trailer.attr('class');
		 //alert($name);
			 jQuery.ajax({
			 url: MyAjax.ajaxurl,
			 type:'POST',
			 dataType: 'html',
			 data: ({action : 'get_movie_review','name':$name}),
			 beforeSend: function() {
				 $('.loader').show();
			  },
			  complete: function(){
...
/****************************On clicking on a movie and getting its dates and time***************************/	
	$(".feature_slider li").on("click", ".book", function(){
		...
		$mypost=$this.attr('href');
		$post=$mypost.substring(1);
		//alert($post);
	...
	//obtain table name, get values that make it at the front end and then send it(movie,date,time)
		function send_table_name(){
			jQuery.ajax({
			  ...
			  success: function(data,state){
				 	//bool to allow**
					console.log("table name data: "+data);
	...
	function results_longpolling(){
		...
		success: function(data,state){
			...
			//keep checking whether mobile money message has been logged
		 		if(data.length==0&&money_checks<15){
						r=setTimeout( function(){results_longpolling();},3500 );
						money_checks++;
...
/********************************************SLIDER************************************/
		...
		var slider=$('.bxslider').slick({
			  ...
			  onAfterChange:function(){ 
					...
					$('.slide_images').css('transform','scale(1,1)');
					$('.focus_slider').fadeOut();
					var $next=$('.slick-active').next();
					var $attr=$next.next().children('div').attr('attr');
					$next.next().css('z-index',6);
					$next.next().next().css('z-index',0);
					$next.next().css('position','relative');
					$next.css('transform','scale(1,1)');
					$next.next().children('img').css('transform','scale(1.3,1.3)');
					$next.next().next().children('img').css('transform','scale(1,1)');
					$next.next().next().next().children('img').css('transform','scale(1,1)');
					//z-index: 5;
